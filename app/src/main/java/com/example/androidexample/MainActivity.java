...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        messageText.setText("Hello Worlds");
        messageText.setTypeface(null, Typeface.BOLD);
        messageText.setTextColor(Color.parseColor("#FFEB3B"));
        messageText.setGravity(Gravity.CENTER);

        View layout = findViewById(R.id.main_layout);       // links to main activity XML
        layout.setBackgroundColor(Color.parseColor("#FF0000"));

        TextView messageText = findViewById(R.id.main_msg_txt);     // always visible
        TextView buttonText = findViewById(R.id.button_msg_txt);    // toggled visibility
        Button button1 = findViewById(R.id.buttons);

        button1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (buttonText.getVisibility() == View.VISIBLE) {
                    buttonText.setVisibility(View.GONE);               // will hide if showing
                } else {
